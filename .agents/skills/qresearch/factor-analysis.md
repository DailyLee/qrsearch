...
充分利用 events：`validate-events` / `sample_profile` 有哪些年就规划哪些年；**勿因扫描窗截断或年未过完丢弃**。较短年默认可进 train（或单独 validate）；**holdout final 须另留**（≥1 自然年，厚度见 quality-gates G9*）。切分细节见 [backtest-optimize.md](backtest-optimize.md)。
