...
环境 ok?（含 exit 日语义：参考退出 → 无 exit_intent）
  no  → 停，提示数据/依赖/语义
  yes → 有 evaluation 协议且过 G9* 切分厚度?
          no  → 先写 evaluation / 按 sample_profile 划 train·validate·holdout（backtest-optimize）
...
- 未写评估协议就网格搜参；`--csv` 与 `evaluation` 角色年不一致（G9e）  
- holdout / validate / stress 参与搜参，或看完 OOS 再改参 / 改网格边界  
- 无 final holdout 或 holdout 过薄仍称 OOS 达标；用 stress 代替 final  
- `Y_all≥4` 无 validate 且无用户跳过就定稿；单年 train 正式搜参/promote  
