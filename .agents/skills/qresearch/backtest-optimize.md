...
回测的日频开盘成交使用 zer0share **当日历史** `up_limit` / `down_limit`：开盘触及涨停不买、触及跌停不卖；停牌不成交。不要把它解读为盘口模拟——引擎不模拟排队、开板时点或盘口。因子 IC 与理论 forward return **不使用**这组成交过滤；它们用于价格关系证据，执行可成交性由 backtest 单独检验。
