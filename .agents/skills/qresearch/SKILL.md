...
7. 因子中性对照必做一次；信号默认原始特征。先写 `evaluation` 再搜参（切分厚度 G9*：正式 train≥2 年、holdout final≥1 年且事件够厚；无固定 7:3）。  
8. 只在 train 搜参；`--csv` 对齐角色年；一次一类旋钮；禁 `sweep×sensitivity`；`n_trials_assumed` ≥ 格点数（见 backtest-optimize）。  
...
```
Task Progress:
- [ ] 0. 环境：ping + 列出全部事件年 → validate-events → 澄清 exit 日语义 → evaluation 切分（过 G9*）
- [ ] 1. 因子分析（仅 train；raw+preprocess；shape；区间则 band-ic；密度预估）→ decision
- [ ] 2. qr config new → 写信号（密度自觉）→ strategy_design decision
- [ ] 3. 训练年 research → analyze trades → quality-gates（失败则回 2 或改 risk）
- [ ] 4. （默认）optimize|sweep（仅 train）→ apply-best（候选）→ research → analyze trades → 闸门
- [ ] 5. sensitivity（含 cost 维）→ apply-best（候选）→ research → 闸门 → 冻结
- [ ] 6. validate（若有）→ holdout final（必做）→ holdout_stress? → decisions
- [ ] 7. 全样本 research（仅披露）→ full_sample decision → 向用户总结
```

顺序意图：**协议/切分 → 信号 → 诊断/闸门 → 分层搜参 → 闸门 → 冻结 → OOS**。
