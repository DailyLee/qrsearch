...
def pipeline_research(
    config_path: str | Path, run_id: str | None = None, n_trials_assumed: int | None = None
) -> dict[str, object]:
    return run_research_strategy(config_path, run_id=run_id, n_trials_assumed=n_trials_assumed)


def _frozen_strategy_inputs(config_path: str | Path) -> tuple[object, object, object, Path]:
    config = _load_config(config_path)
    materialized = materialize_research(config_path)
    run_dir = Path(get_settings().runs_dir) / str(materialized["run_id"])
    _, dataset, _ = _load_frozen_run(run_dir)
...
def pipeline_optimize(config_path: str | Path, *, feature: str | None = None, side: str = "auto", keep_frac: str = "0.1,0.2,0.3,0.4", n_trials: int | None = None) -> dict[str, object]:
    config, frame, panel, run_dir = _frozen_strategy_inputs(config_path)
...
def pipeline_sweep(config_path: str | Path, *, set_specs: list[str], metric: str = "sharpe", max_grid: int = 64) -> dict[str, object]:
    config, frame, panel, run_dir = _frozen_strategy_inputs(config_path)
