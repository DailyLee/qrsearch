...
- **Train**: years for optimize / sweep / sensitivity / WF（≥2 calendar years for formal WF；可含截断年）。
- **Validate**: optional freeze window；**never** tune.
- **Holdout final** (`stage=holdout`)：终测；**never** tune.
- **Holdout stress** (`stage=holdout_stress`)：压力年；差≠机械否决；只归因。
- **Full**：disclose only；须含全部可用事件年；不调参、不单独 promote.
- **充分利用 events**：按 `sample_profile.years` / `years_span` 覆盖选年；勿因截断/年未过完丢弃；statement 起止来自产物。
- Single-year optimize → exploratory only.
